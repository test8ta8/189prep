...
DECLARE
    ...
    v_user_obj jsonb;
    v_correct_obj jsonb;
    v_ua text; v_ub text; v_ca text; v_cb text;
...
BEGIN
    ...
    IF v_question.question_type = 'multipart_ab' THEN
        BEGIN
            v_user_obj := p_user_answer::jsonb;
            v_correct_obj := v_correct_ans::jsonb;
            v_ua := lower(trim(coalesce(v_user_obj->>'a', '')));
            v_ub := lower(trim(coalesce(v_user_obj->>'b', '')));
            v_ca := lower(trim(coalesce(v_correct_obj->>'a', '')));
            v_cb := lower(trim(coalesce(v_correct_obj->>'b', '')));

            IF v_ua != '' AND v_ub != '' AND v_ua = v_ca AND v_ub = v_cb THEN
                v_is_correct := true;
            END IF;
        EXCEPTION WHEN OTHERS THEN v_is_correct := false; END;

    ELSIF v_question.question_type = 'written' THEN
        v_valid_answers := string_to_array(lower(v_correct_ans), ',');
        FOREACH v_elem IN ARRAY v_valid_answers
        LOOP
            IF lower(trim(p_user_answer)) = trim(v_elem) THEN
                v_is_correct := true;
                EXIT;
            END IF;
        END LOOP;
